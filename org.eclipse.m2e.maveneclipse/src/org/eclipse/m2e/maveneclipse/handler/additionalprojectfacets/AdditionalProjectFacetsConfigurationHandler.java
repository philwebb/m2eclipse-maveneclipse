...
/**
 * A {@link ConfigurationHandler} that deals with <tt>additionalProjectFacets</tt> from
 * the <tt>maven-eclipse-plugin</tt> configuration.
 *
 * @author Alex Clarke
 * @author Phillip Webb
 */
public class AdditionalProjectFacetsConfigurationHandler extends
	...
	@Override
	protected void handle(MavenEclipseContext context, ConfigurationParameter parameter)
			throws Exception {
		...
		for (ConfigurationParameter child : parameter.getChildren()) {
			...
			if (projectFacet != null) {
				IProjectFacetVersion projectFacetVersion = projectFacet.getVersion(child
						.getValue());
				...
				if (!facetedProject.hasProjectFacet(projectFacetVersion)) {
					if (facetedProject.hasProjectFacet(projectFacet)) {
						...
						actions.add(new IFacetedProject.Action(
								IFacetedProject.Action.Type.INSTALL, projectFacetVersion,
								config));
					}
				}
			}
	...
	private FacetConfigProvider getFacetConfigProvider(IProjectFacet facet)
			throws CoreException {
		IConfigurationElement[] configurationElements = Platform.getExtensionRegistry()
				.getConfigurationElementsFor(FACET_CONFIG_EXTENSION);
		for (IConfigurationElement configurationElement : configurationElements) {
			String facetId = configurationElement.getAttribute("facetid");
			if (facet.getId().equals(facetId)) {
				return (FacetConfigProvider) configurationElement
	...
	protected IFacetedProject createFacetedProject(MavenEclipseContext context)
			throws CoreException {
		return ProjectFacetsManager.create(context.getProject(), true,
				context.getMonitor());
	}
