...
/**
 * {@link AbstractProjectConfigurator Project configurator} that applies configuration
 * defined by the <tt>maven-eclipse-plugin</tt> to m2e eclipse projects.
 *
...
public class MavenEclipseProjectConfigurator extends
	...
	private static Logger log = LoggerFactory
			.getLogger(MavenEclipseProjectConfigurator.class);
	...
	public void configureRawClasspath(ProjectConfigurationRequest request,
		...
		// We are not actually configuring the raw classpath but we want to run after m2e
		// has configured
		...
		for (Plugin plugin : request.getMavenProject().getBuildPlugins()) {
			if (isMavenEclipsePlugin(plugin)) {
				log.debug("Configuring m2e project for " + GROUP_ID + ":" + ARTIFACT_ID
						+ " plugin");
	...
	private boolean isMavenEclipsePlugin(Plugin plugin) {
		return (GROUP_ID.equals(plugin.getGroupId()) && ARTIFACT_ID.equals(plugin
				.getArtifactId()));
	}

	private void configure(ProjectConfigurationRequest request, IProgressMonitor monitor,
			Plugin plugin) throws CoreException {
		if (this.handlers == null) {
			this.handlers = new ConfigurationHandlers();
		}
		MavenEclipseContext context = new DefaultMavenEclipseContext(request, monitor,
				plugin);
		try {
			this.handlers.handle(context);
		}
		catch (Exception ex) {
			if (ex instanceof CoreException) {
				throw (CoreException) ex;
			}
			String msg = NLS.bind(Messages.mavenEclipseProjectConfiguratorError,
					ex.getMessage());
			log.error(msg, ex);
			throw new CoreException(new Status(IStatus.ERROR,
					MavenEclipsePlugin.PLUGIN_ID, -1, msg, ex));
		}
	}
